...
ZOOM_STEP = 0.9
SHELL_OFFSET_MIN = -0.2
SHELL_OFFSET_MAX = 0.2
...
def d_to_w(d: float) -> float:
    """Convert a physical offset distance to the precomputed surface parameter."""
    return 2.5 * d + 0.5


def shell_offset_to_w(value: str) -> float:
    """Validate a shell offset distance and convert it to the surface parameter."""
    try:
        offset = float(value)
    except ValueError as exc:
        raise ValueError("Shell offset distance must be a number.") from exc

    if not SHELL_OFFSET_MIN <= offset <= SHELL_OFFSET_MAX:
        raise ValueError(
            f"Shell offset distance must be between "
            f"{SHELL_OFFSET_MIN:.2f} and {SHELL_OFFSET_MAX:.2f}."
        )
    return d_to_w(offset)


def shell_offset_to_slider(offset: float) -> int:
    ratio = (offset - SHELL_OFFSET_MIN) / (SHELL_OFFSET_MAX - SHELL_OFFSET_MIN)
    return round(max(0.0, min(1.0, ratio)) * SLIDER_STEPS)


def slider_to_shell_offset(value: int) -> float:
    ratio = max(0, min(SLIDER_STEPS, value)) / SLIDER_STEPS
    return SHELL_OFFSET_MIN + ratio * (SHELL_OFFSET_MAX - SHELL_OFFSET_MIN)
...
# ══════════════════════════════════════════════════════════
DENSITY_PARAMS = {
    "SchwarzP": {"a": 1.1029, "b": 0.9731},
    "IWP":      {"a": 1.5328, "b": 0.9455},
    "Gyroid":   {"a": 1.4134, "b": 0.9591},
    "Diamond":  {"a": 0.8603, "b": 0.9491},
}
...
class OCCTWidget(QWidget):
    ...
    def _initialize(self):
        try:
            win = create_occt_window(self, self.display_connection)
            ...
            QMessageBox.critical(
                self,
                "3D viewport initialization failed",
                f"The OCCT viewport could not be initialized.\n\n{exc}",
            )
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        lbl_offset_sec = QLabel("Offset distance  d")
        lbl_offset_sec.setStyleSheet(
            "color:#1565C0; font-size:11px; font-weight:bold; margin-top:2px;"
        )
        vs.addWidget(lbl_offset_sec)

        self.e_t = self._line_edit("0.00")
        offset_validator = QDoubleValidator(
            SHELL_OFFSET_MIN, SHELL_OFFSET_MAX, 4, self.e_t
        )
        offset_validator.setNotation(QDoubleValidator.StandardNotation)
        self.e_t.setValidator(offset_validator)
        self.e_t.setToolTip(
            f"Physical offset distance from {SHELL_OFFSET_MIN:.2f} "
            f"to {SHELL_OFFSET_MAX:.2f}."
        )
        offset_slider_row, self.slider_shell_offset = _make_slider_row(
            "#4A90D9",
            f"{SHELL_OFFSET_MIN:.2f}",
            f"{SHELL_OFFSET_MAX:.2f}",
        )
        self.slider_shell_offset.setValue(shell_offset_to_slider(0.0))
        vs.addWidget(self._param_row("d :", self.e_t, offset_slider_row))
        ...
        # Connect signals
        self.slider_shell_offset.valueChanged.connect(self._on_slider_shell_offset)
        self.e_t.editingFinished.connect(self._on_edit_shell_offset)
    ...
    def _on_slider_shell_offset(self, value: int):
        if self._syncing:
            return
        offset = slider_to_shell_offset(value)
        self.e_t.setText(f"{offset:.4f}")

    def _on_edit_shell_offset(self):
        if self._syncing:
            return
        try:
            offset = float(self.e_t.text())
        except ValueError:
            return

        offset = max(SHELL_OFFSET_MIN, min(SHELL_OFFSET_MAX, offset))
        self._syncing = True
        try:
            self.e_t.setText(f"{offset:.4f}")
            self.slider_shell_offset.setValue(shell_offset_to_slider(offset))
        finally:
            self._syncing = False
    ...
    def _current_params(self) -> dict:
        ...
        is_shell = t.endswith("-Shell")
        is_solid = t.endswith("-Solid")
        shell_w = shell_offset_to_w(self.e_t.text()) if is_shell else 0.0
        return {
            'type':      t,
    ...
    def _on_generate(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Invalid offset distance", str(exc))
            self.e_t.setFocus()
            return
    ...
    def _on_export(self):
        ...
        try:
            ...
            current_params = None
        if self._last_params is not None and current_params != self._last_params:
            QMessageBox.warning(
